...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    %%% dE/dZj = Yj - Tj deriv of cross-entropy cost function
    ...
    btl_count = btl_count + 1;
    if (mod(btl_count, 100) == 0) 
    ...
    % BACK PROPAGATE.
    %% OUTPUT LAYER.
    %%% dE/dWij = Yi * dE/dZj = Yi * error_deriv = Yi * (Yj - Tj)
    hid_to_output_weights_gradient =  hidden_layer_state * error_deriv';
    output_bias_gradient = sum(error_deriv, 2);
    %%% dE/dYi = sum(Wij * dE/dZj) = sum(Wij * Yj(1-Yj)*dE/dYj)) = 
    %%%    sum(Wij * dE/dYj) * (Yj * (1 - Yj))
