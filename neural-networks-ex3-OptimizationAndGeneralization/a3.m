...
function test_gradient(model, data, wd_coefficient)
  ...
  for i = 1:100,
    ...
    for contribution_index = 1:8,
      modl = theta_to_model(base_theta + theta_step * contribution_distances(contribution_index));
      temp = temp + loss(modl, data, wd_coefficient) * contribution_weights(contribution_index);
    end
    fd_here = temp / h;
  ...
  fprintf('Gradient test passed. That means that the gradient that your code computed is within 0.001%% of the gradient that the finite difference\n');
  fprintf('approximation computed, so the gradient calculation procedure is probably correct (not certainly, but probably).\n');
end
...
function ret = loss(model, data, wd_coefficient)
  ...
  %   Each column describes a different data case. It is a one-of-N encoding of the class, 
  %   i.e. one element in every column is 1 and the others 0.
  ...
  % The following three lines of code implement the softmax. However, it's written differently from what the lectures say.
  ...
  % The exponential in the lectures can lead to really big numbers, which are fine in mathematical equations, 
  % but can lead to all sorts of problems in Octave. Octave isn't well prepared to deal with really large numbers, like 10 to the power 1000.  
  %   we subtract log(sum(exp of class_input)) to get properly normalized log class probabilities.  size: <1> by <number of data cases>
  ...
  %   sum() is a row vector 1 x #dataPts with each element = sum of log class probabilities for that data point
  %   we compute the average softmax classification loss/cost over all data points
  classification_loss = -mean(sum(log_class_prob .* data.targets, 1));
   % weight decay loss. very straightforward: E = 1/2 * wd_coefficient * theta^2
  wd_loss = sum(model_to_theta(model).^2)/2 * wd_coefficient;
  % total loss (error) is the sum of the classification and the weight-decay loss.
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % ================= Copy of the code from loss() to compute needed values =================
  % Before we can calculate the loss, we need to calculate a variety of intermediate values, like the state of the hidden units.
  %   input to the hidden units, i.e. before the logistic.   size: <number of hidden units> by <number of data cases>
  hid_input = model.input_to_hid * data.inputs;
  %   output of the hidden units, i.e. after the logistic.   size: <number of hidden units> by <number of data cases>
  ...
  %   probability of each class. Each column (i.e. each case) sums to 1.  size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob); 
  %  ================= End of the copy of the code from loss() to compute these =================
  ...
  % This is the only function that you're expected to change. Right now, it just returns a lot of zeros, which is obviously not the 
  % correct output. Your job is to replace that by a correct computation.
  % 
  % (10 x nhid)' * (10 x ndata) = (nhid x ndata)
  d_cl_by_d_h     = model.hid_to_class' * (class_prob - data.targets);
  % ((nhid x ndata) .* (nhid x ndata)) * (256 x ndata)' = (nhid x 256)
  d_cl_by_d_i2h   = (d_cl_by_d_h .* (hid_output .* (1-hid_output))) * data.inputs';
  i2h_class_error = d_cl_by_d_i2h / size(data.targets, 2);
  
  % (10 x ndata) * (nhid x ndata)' = (10 x nhid)
  d_cl_by_d_h2c   = (class_prob - data.targets) * hid_output';
  h2c_class_error = d_cl_by_d_h2c / size(data.targets, 2);
  
  i2h_wd_error = wd_coefficient * model.input_to_hid;
  h2c_wd_error = wd_coefficient * model.hid_to_class;
  
  ret.input_to_hid = (i2h_class_error + i2h_wd_error);
  ret.hid_to_class = (h2c_class_error + h2c_wd_error);
end
...
function ret = theta_to_model(theta)
  % This function takes a model (or gradient) in the form of one long vector (maybe produced by model_to_theta), and restores it to the 
  % structure format, i.e. with fields .input_to_hid and .hid_to_class, both matrices.
